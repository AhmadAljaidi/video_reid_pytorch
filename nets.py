...
class Net(nn.Module):
    ...
    def forward(self, x1, x2, steps, hidden=None):
        ...
        left_output  = left_output.view(-1, 128)  # (B, 128)
        right_output = right_output.view(-1, 128) # (B, 128)

        l_id = self.classifierLayer(left_output)
        r_id = self.classifierLayer(right_output)

        l_id = self.log_softmax(l_id)
        r_id = self.log_softmax(r_id)

        return left_output, right_output, l_id, r_id, hidden
